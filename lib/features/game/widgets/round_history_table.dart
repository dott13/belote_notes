          ...
          builder: (context, constraints) {
            return SingleChildScrollView(
              ...
              child: ConstrainedBox(
                ...
                child: Center(child: _buildTable()),
              ),
            );
          },
        ),
  ...
  Widget _buildTable() {
    return DataTable(
      ...
      columns: [
        ...
        if (onDeleteRound != null) const DataColumn(label: Text('')),
      ],
      rows: [
        ...rounds.map((round) {
        ...
        DataRow(
          color: WidgetStateProperty.all(Colors.grey.withValues(alpha: 0.1)),
          cells: [
            const DataCell(
            ...
            DataCell(
              Text(
                totalScores.values
                    .map((s) => s < 0 ? 0 : s) // negative totals count as 0
                    .fold(0, (a, b) => a + b)
                    .toString(),
                style: const TextStyle(fontWeight: FontWeight.bold),
              ),
            ),
            if (onDeleteRound != null) const DataCell(SizedBox.shrink()),
          ],
        ),
      ],
    );
